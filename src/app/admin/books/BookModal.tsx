...
export function BookModal({ open, onOpenChange, book, onSubmit, loading = false, categoryOptions, categorySearch, setCategorySearch }: Props) {
  const form = useForm({
    defaultValues: {
      title: '',
      author: '',
      ...
      <Dialog.Content maxWidth='500px'>
        ...
        >
          <Box className='space-y-4'>
            <form.Field name='title'>{(field) => <InputField field={field} label='Title' icon={<Icon icon='mdi:book' />} />}</form.Field>

            <form.Field name='author'>{(field) => <InputField field={field} label='Author' icon={<Icon icon='mdi:account-edit' />} />}</form.Field>

            <form.Field name='publisher'>
              {(field) => <InputField field={field} label='Publisher' icon={<Icon icon='mdi:office-building' />} />}
            </form.Field>

            <form.Field name='isbn'>{(field) => <InputField field={field} label='ISBN' icon={<Icon icon='mdi:barcode' />} />}</form.Field>

            <form.Field name='year'>
              {(field) => <InputField field={field} type='number' label='Year' icon={<Icon icon='mdi:calendar' />} />}
            </form.Field>

            <form.Field name='stock'>
              {(field) => <InputField field={field} type='number' label='Stock' icon={<Icon icon='mdi:counter' />} />}
            </form.Field>
