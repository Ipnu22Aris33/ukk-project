...
export const PATCH = handleApi(async ({ params }) => {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    // Validasi: Hanya reservasi yang masih 'pending' yang bisa dicancel
    if (reservation.status !== 'pending') {
      throw new BadRequest(`Reservasi tidak bisa dibatalkan karena sudah ${reservation.status}`);
    }

    const now = new Date();

    // 2. Update status reservasi menjadi CANCELLED
    ...
    // 3. Reversal Stok: Kurangi Reserved, Kembalikan ke Available
    await tx
      .update(books)
      .set({
